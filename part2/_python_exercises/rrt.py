...
# Set to True for the solution to the bonus question.
BONUS = False
...
class OccupancyGrid(object):
  ...
  def draw(self):
    plt.imshow(np.transpose(self._original_values), interpolation='none', origin='lower',
...
def rrt(start_pose, goal_position, occupancy_grid):
  ...
  for _ in range(MAX_ITERATIONS): 
    ...
    for n, d in potential_parent:
      ...
      continue
    if BONUS:
      v, dcost = adjust_pose(u, position, occupancy_grid)
    else:
